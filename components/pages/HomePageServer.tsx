...
import { getAllToolSlugs, getToolConfig } from '@/lib/toolConfig';
    ...
    return (
        <>
            ...
            <div className="min-h-screen flex flex-col">
                {/* We use a thin client wrapper if we want to handle language state on clients, 
            but for SEO, we render the header with the server-passed lang */}
                <Header lang={lang} setLang={() => { }} onNavigate={() => { }} />
                    ...
                    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 text-center">
                        ...
                        <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-10 sm:mb-16">
                            {[
                                ...
                                <div
                                    key={idx}
                                    className="inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-white/80 backdrop-blur-sm border border-gray-101 rounded-2xl text-gray-700 text-xs sm:text-sm font-semibold shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.02]"
                                >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                            {toolSlugs.map((slug) => (
                                <Link
                                    key={slug}
                                    href={`/${slug}`}
                                    className="group bg-white border-2 border-gray-101 rounded-2xl sm:rounded-3xl p-4 sm:p-6 hover:border-canada-red hover:shadow-xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
                                >
                    ...
                    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
                        <div className="grid md:grid-cols-3 gap-6 sm:gap-8">
                            {[
                                ...
                                <div
                                    key={idx}
                                    className="bg-white/80 backdrop-blur-sm border border-gray-101 rounded-3xl p-6 sm:p-8 shadow-sm hover:shadow-md transition-shadow"
                                >
