...
export const LegalPdfGuide: React.FC<GuideProps> = ({ lang }) => {
    ...
    const schema = {
        ...
        "about": [
            { "@type": "Thing", "name": "Solicitor-Client Privilege" },
            { "@type": "Thing", "name": "PIPEDA Compliance" },
            { "@type": "Thing", "name": "Legal Document Security" }
        ]
    };
    ...
    return (
        ...
        >
            ...
            <div className="w-full py-12">
                ...
                <div className="bg-slate-900 text-white rounded-xl md:rounded-[2rem] p-6 md:p-12 text-center shadow-xl">
                    <h3 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6">{t.cta}</h3>
                    <Link href={`/${lang}`} className="inline-block bg-white text-slate-900 hover:scale-105 transition-all px-6 md:px-8 py-3 md:py-4 rounded-full font-bold text-base md:text-lg border-2 border-transparent hover:border-white hover:bg-slate-900 hover:text-white">
                        {t.ctaBtn}
                    </Link>
                </div>
